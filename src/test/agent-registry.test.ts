...
const ALICE_PRIVATE_STATE_ID = 'AliceAgentRegistryState';
...
describe('Agent Registry Contract', () => {
  ...
  it('Deploys the agent registry contract', async () => {
    logger.info('Creating private state...');

    const deployed: any = await (deployContract as any)(aliceProviders, {
      compiledContract: CompiledAgentRegistryContract,
      privateStateId: ALICE_PRIVATE_STATE_ID,
      initialPrivateState: {},
      args: [],
      witnesses: {
        agentCapabilities: (_context: any) => {
          return new TextEncoder().encode('default-capabilities');
        },
      },
    });
    ...
    expect(state.agentCount).toEqual(0n);
  });

  it('Registers an agent', async () => {
    const agentId = new Uint8Array(32).fill(0);
    agentId[31] = 1;
    ...
    const state = await queryLedger(aliceProviders);
    expect(state.agentCount).toEqual(1n);
  });
});
