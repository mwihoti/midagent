...
const ALICE_PRIVATE_STATE_ID = 'AliceMarketplaceState';
...
describe('Marketplace Contract', () => {
  ...
  it('Deploys the marketplace contract', async () => {
    ...
    const deployed: any = await (deployContract as any)(aliceProviders, {
      compiledContract: CompiledMarketplaceContract,
    ...
    expect(state.intentCount).toEqual(0n);
  });

  it('Submits an intent', async () => {
    ...
    await (submitCallTx as any)(aliceProviders, {
      ...
      contractAddress,
      privateStateId: ALICE_PRIVATE_STATE_ID,
      circuitId: 'submitIntent',
      args: [intentId],
    });

    const state = await queryLedger(aliceProviders);
    expect(state.intentCount).toEqual(1n);
  });
});
